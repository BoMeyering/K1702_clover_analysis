...
class ObjTrainer:
    ...
    def _train_step(self, batch):
        ...
        imgs = imgs.to(self.device)
        # Ensure each target is a dict with 'bbox' and 'cls', moved to device
        targets = {
            k: [j.to(self.device) for j in v] for k, v in targets.items()
        }
        # targets = [{k: v.to(self.device) for k, v in t.items()} for t in targets]

        self.optimizer.zero_grad()
        loss_dict = self.model(imgs, targets)  # this works now (not anymore)
        loss = loss_dict["loss"]
        self.optimizer.step()
    ...
    def _val_step(self, batch):
        ...
        targets['img_scale'] = torch.tensor([torch.tensor([1]) , torch.tensor([1])])
        targets['img_size'] = torch.tensor([[1024, 1024] , [1024, 1024]])

        loss_dict = self.model(imgs, targets)  # Pass targets to get loss dict
        loss = loss_dict["loss"]
        return loss